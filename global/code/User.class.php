...
class User
{
	 ...
	 * string.
	 *
	 * Internally, a user can logout by passing a "?logout" query string to any page in CloudserveForms.
	 *
	...
	public function logout($message_flag = "")
	{
		...
		if (Sessions::exists("admin")) {
			...
			if (!empty($message_flag)) {
				// empty sessions, but be nice about it. Only delete the CloudserveForms namespaced sessions - any other
	 ...
	 *
	 * Should be called on ALL CloudserveForms pages - including modules.
	 *
