  ...
  {else}
    ...
    {if isset($error_code)}
      <p>
        ...
        <a href="https://docs.forms.cloudservetechcentral.com/api/v2/error_codes/#{$error_code}" target="_blank" />{$LANG.phrase_error_learn_more}</a>
      </p>
    {/if}
    ...
    {if isset($error_codes)}
      <p>
        ...
        {foreach from=$error_codes item=row}
          <a href="https://docs.forms.cloudservetechcentral.com/api/index.php?page=error_codes#{$row}" target="_blank" />{$row}</a>
        {/foreach}
      </p>
    {/if}
